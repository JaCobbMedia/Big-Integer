...
longInt createBigInteger(char*);
void printInteger(longInt);
void trimFrontZeros(longInt);
void freeBigInteger(longInt*);
longInt addition(longInt, longInt);
longInt subtraction(longInt, longInt);
...
void trimFrontZeros(longInt number)
{
    ...
    while (temp->number == 0 && temp->next !=NULL)
    {
        ...
        free (temp);
        temp = number->head;
        temp->prev = NULL;  // SITOS EILUTES TRUKO
    }
}
...
int main ()
{
	...
	c = createBigInteger ("50");
    d = createBigInteger ("-563475854321");
	a = createBigInteger ("62545");
	b = createBigInteger ("10000");
	printf("%d\n",checkError(b));
	printInteger(a);
	printf(" / ");
	printInteger(b);
